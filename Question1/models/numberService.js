...
class NumberService {
  ...
  async fetchNumbers(numberType) {
    ...
    // Determine which API to call based on the number type
    switch (numberType) {
      case 'p':
    ...
    try {
      ...
      // Set timeout to 500ms as per requirements
      ...
      if (response.data && response.data.numbers) {
        // The API returns numbers as a string or an array
      ...
      // Log the error but return an empty array to continue processing
      console.error(`Error fetching numbers: ${error.message}`);
      
      // For testing purposes, return some mock data if the API fails
      if (numberType === 'e') {
